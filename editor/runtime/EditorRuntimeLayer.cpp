...
EditorRuntimeLayer::EditorRuntimeLayer(std::shared_ptr<Scene> scene)
    : Layer("EditorRuntime"), m_scene(std::move(scene))
{
...
{
    if (m_playing || !m_viewportHovered || !Input::isMouseButtonDown(MouseButton::Right))
        return;
...
void EditorRuntimeLayer::setViewportSize(uint32_t width, uint32_t height)
{
    if (width < 2 || height < 2 || (width == m_lastWidth && height == m_lastHeight))
        return;
...
void EditorRuntimeLayer::onRender(Renderer& renderer)
{
    ...
    if (m_lastWidth == 0 || m_lastHeight == 0)
        setViewportSize(Application::get().getWindow().getWidth(),
